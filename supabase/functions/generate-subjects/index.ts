...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Generate 3 concise email subject lines for a "${category}" preventive health campaign in ${lang === 'hi' ? 'Hindi' : 'English'}.
...
Example: {"subjects": ["Is your heart trying to tell you something?", "3 signs you shouldn't ignore", "Take control of your health today"]}`;
    ...
    console.log('Calling Lovable AI for subject generation...');
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are a healthcare marketing expert. Always respond with valid JSON only.' },
